    ...
    return (
        <form onSubmit={SubmitHandler} className="flex flex-col w-full gap-y-4 mt-6">
            ...
            <label className="w-full relative">
                ...
                <sup className="text-pink-200">*</sup></p>
                <input required type={showPass?("text"):("password")} value={formData.password} onChange={changeHandler} placeholder="Enter Password" name="password"
                className="bg-richblack-800 rounded-[0.5rem] text-richblack-5 w-full p-[12px]"
                />

                <span onClick={()=>setShowpass(!showPass)} className="absolute right-3 top-[38px] cursor-pointer">
